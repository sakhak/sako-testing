...
const App: React.FC = () => {
  const [currentPage, setCurrentPage] = useState<'home' | 'about' | 'products' | 'solutions' | 'cases' | 'news' | 'support' | 'faq' | 'service-centers' | 'training-video' | 'easy-solar-kit' | 'energy-storage' | 'download' | 'contact'>('home');
  ...
  const navigateTo = (page: string, subKey?: string) => {
    if (page === 'home') window.location.hash = '';
      ...
      const catParam = catVal !== 'Products' ? `?category=${encodeURIComponent(catVal)}` : '';
      window.location.hash = `products-page${catParam}`;
    } else if (page === 'solutions') {
      const solVal = subKey || 'Solutions';
      const solParam = solVal !== 'Solutions' ? `?type=${encodeURIComponent(solVal)}` : '';
    ...
    else if (page === 'service-centers') window.location.hash = 'service-centers-page';
    else if (page === 'training-video') window.location.hash = 'training-video-page';
    else if (page === 'easy-solar-kit') window.location.hash = 'easy-solar-kit-page';
    else if (page === 'energy-storage') window.location.hash = 'energy-storage-page';
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-white">
      ...
      <main className="flex-grow pt-24 lg:pt-32">
        ...
        {currentPage === 'products' && <ProductsPage initialCategory={selectedCategory || 'Products'} />}
        {currentPage === 'solutions' && <SolutionsPage type={selectedSolution || 'Solutions'} />}
